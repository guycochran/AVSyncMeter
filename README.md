...
Frame rate, flash/audio sensitivity, central target size, pairing window (±400 ms default), optional manual thresholds, stability threshold, outlier MAD k, VU history window (1–90 s, default 90 s), and a persisted known-correction (calibration). Default calibration is **0 ms = none applied**, not “sensor latency is zero”.
