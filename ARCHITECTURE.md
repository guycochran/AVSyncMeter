...
SwiftUI (`MeasurementView`) is dark, low-decoration, venue-friendly: preview + target, huge AUDIO EARLY/LATE + ms, recommended delay, fps/frames, stats, SYNC STABLE/UNSTABLE, WALK ms/beep (green only if walk is flat and SPAN is tight), live LUMA/MIC needles plus a scrolling 1–90 s (default 90 s) peak-held history strip (newest at the right), Start/Stop/Reset. Capture footer is `%.2f` plus NTSC vs integer. Version string is on the header. Settings, Diagnostics, and a Phase 2 test signal (generated PCM beep, not the silent-switch click) are sheets.
