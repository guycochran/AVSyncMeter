...
Unpaired flashes stay in a short queue (not keep-latest of one). A 60 fps measure queue can ingest the next 1 Hz flash before a pairable pulse whose onset is still inside ±400 ms of the previous flash; dropping that flash as extra was zero pairs with FLASH+AUDIOPULSE marks. Latest pairable pulse still wins; overlapping speech is never queued. They pair only if `|audio − video|` is inside `maxPairOffsetSeconds` (default ±400 ms). Isolated house hits pair on onset even if the old isBeepLike duration gate was false (Harkwood 1001 ms / 66.7 ms 3 kHz, or a 200–400 ms periodic tone). Overlapping/ongoing speech never pairs. Otherwise the older head expires unpaired, so a 220–350 ms ring-down replica cannot steal the next 1 Hz flash. `pairingWindowSeconds` (default 400 ms) is how long a lone event waits. No accumulating pairing debt.
