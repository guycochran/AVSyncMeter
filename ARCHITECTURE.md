...
Unpaired flashes stay in a short queue (not keep-latest of one). A 60 fps measure queue can ingest the next 1 Hz flash before a beep-like pulse whose onset is still inside ±400 ms of the previous flash; dropping that flash as extra was zero pairs with FLASH+AUDIOPULSE marks. Latest beep-like pulse still wins; voice/chatter is never queued. They pair only if `|audio − video|` is inside `maxPairOffsetSeconds` (default ±400 ms). Voice-like pulses never pair, so extra speech in the window cannot steal the house beep. Otherwise the older head expires unpaired, so a 220–350 ms ring-down replica cannot steal the next 1 Hz flash. `pairingWindowSeconds` (default 400 ms) is how long a lone event waits. No accumulating pairing debt.
...
| `Engine/SyncMeasurementEngine.swift` | Flash queue + one beep-like pulse; pair inside ±400 ms |
