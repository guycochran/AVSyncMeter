...
Stage-noise: emit isolated 1 Hz beeps (15–80 ms PA smear then quiet, even if the old sharp/short isBeepLike gate was false). Overlapping/ongoing energy is speech and is not an event; a 1 kHz overlay can still win while speech is held. 400 ms mask after a real beep; quiet re-arm must not fire on the next syllable.

No pitch detection (duration + high-band energy only).

## Pairing (`SyncMeasurementEngine`)

Unpaired flashes stay in a short queue (not keep-latest of one). A 60 fps measure queue can ingest the next 1 Hz flash before a pairable pulse whose onset is still inside ±400 ms of the previous flash; dropping that flash as extra was zero pairs with FLASH+AUDIOPULSE marks. Latest pairable pulse still wins; overlapping speech is never queued. They pair only if `|audio − video|` is inside `maxPairOffsetSeconds` (default ±400 ms). Isolated 1 Hz (smear ≤ 85 ms, quiet after) pairs even if the old isBeepLike gate was false. Overlapping/ongoing speech never pairs. Otherwise the older head expires unpaired, so a 220–350 ms ring-down replica cannot steal the next 1 Hz flash. `pairingWindowSeconds` (default 400 ms) is how long a lone event waits. No accumulating pairing debt.
