...
Unpaired flashes stay in a short queue (not keep-latest of one). A 60 fps measure queue can ingest the next 1 Hz flash before a pairable pulse whose onset is still inside ±400 ms of the previous flash; dropping that flash as extra was zero pairs with FLASH+AUDIOPULSE marks. Earliest pairable pulse wins; a later pulse within ~80 ms of the flash (HDMI embed glued to picture — still useful if a monitor has speakers; unconfirmed on the split-path LCD) must not replace a PA beep at T−80 / T−200. Overlapping speech is never queued. They pair only if `|audio − video|` is inside `maxPairOffsetSeconds` (default ±400 ms). Isolated house hits pair on onset even if the old isBeepLike duration gate was false (Harkwood 1001 ms / 66.7 ms 3 kHz, or a 200–400 ms periodic tone). Overlapping/ongoing speech never pairs. Otherwise the older head expires unpaired, so a 220–350 ms ring-down replica cannot steal the next 1 Hz flash. `pairingWindowSeconds` (default 400 ms) is how long a lone event waits. No accumulating pairing debt.

VU luma/mic and EVT FLASH/AUDIOPULSE/PAIR marks use the same CaptureClock unified seconds as pairing. Wall-clock stamps made 1 Hz spikes look aligned on the strip while `|unified dt| > 400 ms` never paired. EVT marks follow engine ingest, not a VU envelope threshold.
...
| `Engine/SyncMeasurementEngine.swift` | Flash queue + earliest pairable pulse; pair inside ±400 ms |
